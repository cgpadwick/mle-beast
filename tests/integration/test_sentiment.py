...
"""E2E integration test: full pipeline on sentiment_classification data.

This test runs the full pipeline (code -> test -> review -> train -> analyze)
on the sentiment classification dataset with a 75% accuracy target.
...
DATA_DIR = Path(__file__).parent / "sentiment_classification" / "data"
PROJECT_YAML = Path(__file__).parent / "sentiment_classification" / "project.yaml"


@pytest.fixture
def workspace(tmp_path):
    """Create a workspace for the sentiment integration test."""
    ws = tmp_path / "sentiment_workspace"

    creator = WorkspaceCreator(str(ws), quiet=True)
    creator.run()

    # Copy data directory structure (train/{positive,negative}.jsonl, test/...)
    data_dir = ws / "data"
...
def test_sentiment_full_pipeline(workspace):
    """Run full pipeline on sentiment data and assert accuracy >= 0.75."""
    ws = workspace
