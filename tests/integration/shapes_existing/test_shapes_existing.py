...
"""E2E integration test: existing-code pipeline on shapes data.
...
4. Asserts accuracy >= 0.80 and files were modified (not versioned)
...
def workspace(tmp_path):
    """Create a workspace with existing code for the shapes_existing test."""
    ws = tmp_path / "shapes_existing_workspace"

    # Brownfield: user owns the venv. Templates need torch + torchvision.
    prep_brownfield_venv(ws, ["torch", "torchvision"])
...
def test_shapes_existing_pipeline(workspace):
    """Run full pipeline in existing-code mode on shapes data."""
    ws = workspace
