...
"""E2E integration test: existing-code pipeline on churn data.
...
4. Asserts accuracy >= 0.55 and files were modified (not versioned)
...
def workspace(tmp_path):
    """Create a workspace with existing code for the churn_existing test."""
    ws = tmp_path / "churn_existing_workspace"

    # Brownfield: user owns the venv. Build a minimal one with just what
    # the templates import (torch + pandas) instead of the heavyweight
    # ml-frameworks stack WorkspaceCreator would install.
    prep_brownfield_venv(ws, ["torch", "pandas"])
...
def test_churn_existing_pipeline(workspace):
    """Run full pipeline in existing-code mode on churn data."""
    ws = workspace
